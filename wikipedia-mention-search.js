...
// v32: "מקור השם" is reserved for the CURRENT street name only.
// Historical/previous names and their explanations are classified under former names.
(function () {
  ...
  const FORMER_CONTEXT=/(?:בעבר|קודם לכן|לפני כן|תחילה|בראשיתו|בראשיתה|בתקופת המנדט|בימי המנדט|בתקופה העות['׳]?מאנית|בתקופה הבריטית|עד שנקרא|עד ששונה|שמו הקודם|שמה הקודם)/u;
  function quotedHistoricalAlias(sentence,street){
    const quotes=[...sentence.matchAll(/["״']([^"״']{2,60})["״']/gu)].map(m=>clean(m[1]));
    return quotes.some(q=>norm(q)!==norm(street)&&/^(?:רחוב|שדרות|שדרה|דרך|כביש|סמטה|סמטת)?\s*[א-ת]/u.test(q));
  }
  function isFormerNameSentence(sentence,street){
    if(FORMER_CONTEXT.test(sentence))return true;
    if(/(?:נקרא|נקראה)[^.!?]{0,120}["״'][^"״']{2,60}["״']/u.test(sentence)&&quotedHistoricalAlias(sentence,street))return true;
    if(/(?:שונה|הוחלף|הוסב)\s+(?:שמו|שמה)/u.test(sentence))return true;
    return false;
  }
  function originEvidence(list,street){
    const sentence=list.find(s=>{
      if(isFormerNameSentence(s,street))return false;
      return /(?:נקרא|נקראת|קרוי|קרויה|נקראות|נקראים|מכונה)[^.!?]{0,220}(?:על שם|על־שם|על שמו|על שמה|לזכר|מנציח)/u.test(s);
    });
  ...
  function formerEvidence(list,street){
    const out=[];
    const patterns=[
      /(?:נקרא|נקראה)\s+(?:בעבר|קודם לכן|לפני כן|תחילה|בראשיתו|בראשיתה)\s+["״']?([^"״'.;,]{2,48})["״']?/u,
      /(?:נקרא|נקראה)\s+(?:בתקופת המנדט|בימי המנדט|בתקופה הבריטית|בתקופה העות['׳]?מאנית)\s+["״']?([^"״'.;,]{2,48})["״']?/u,
      /(?:שמו|שמה)\s+(?:הקודם|הקודמת)\s+(?:היה|הייתה)\s+["״']?([^"״'.;,]{2,48})["״']?/u,
      /(?:שונה|הוחלף|הוסב)\s+(?:שמו|שמה)\s+מ[־-]?["״']?([^"״'.;,]{2,48})["״']?\s+(?:ל|אל)/u
    ];
    for(const sentence of list){
      for(const p of patterns){const m=sentence.match(p),name=m&&plausibleFormerName(m[1],street);if(name&&!out.some(x=>x.value===name))out.push({value:name,sentence});}
      if(isFormerNameSentence(sentence,street)){
        const aliases=[...sentence.matchAll(/["״']([^"״']{2,60})["״']/gu)].map(m=>plausibleFormerName(m[1],street)).filter(Boolean);
        for(const name of aliases)if(!out.some(x=>x.value===name))out.push({value:name,sentence});
      }
    }
